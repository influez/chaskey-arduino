...
#include "chaskey12.h"
#if CHASKEY12_DEBUG || CHASKEY12_TEST
#include <stdio.h>
#endif
#include <string.h>
#include <assert.h>
...
void chaskey12(uint8_t *tag, const size_t taglen, const uint8_t *m, const size_t mlen, const uint32_t k[4], const uint32_t k1[4], const uint32_t k2[4]) {
  assert(taglen <= CHASKEY12_TAG_SIZE);

  Chaskey12Context context;
  chaskey12_init(&context, k, k1, k2);
  chaskey12_process(&context, m, mlen);
  chaskey12_finish(&context);
...
static inline void chaskey12_bswap(uint32_t l[4]) {
...
static void chaskey12_mix(Chaskey12Context* context, const uint32_t* l) {
  size_t i;
  for (i = 0; i < 4; i++) {
    context->tag[i] ^= l[i];
  }
...
void chaskey12_process(Chaskey12Context* context, const uint8_t* m, size_t len) {
  ...
  for (i = context->len & (CHASKEY12_TAG_SIZE - 1); len > 0; i = 0) {
    if (i == 0 && context->len > 0) {
      permute(context->tag);
    }

    blocklen = CHASKEY12_TAG_SIZE - i;
    ...
    if (i + blocklen == CHASKEY12_TAG_SIZE) {
#if CHASKEY12_DEBUG
      ...
      chaskey12_bswap(context->m.u32);
      chaskey12_mix(context, context->m.u32);
    }
  }
}
...
void chaskey12_finish(Chaskey12Context* context) {
  ...
  size_t i = context->len & (CHASKEY12_TAG_SIZE - 1);
...
#define vectors vectors_(CHASKEY12_ROUNDS)
...
void printHex(const unsigned char *data, int size)
{
  ...
  for (i = 0; i < 64; i++) {
    ...
    printf("Chaskey12: "); printHex(tag, 8); printf("\t");
    printf("Test Vector: "); printHex(vectors[i], 8); printf("\n");
